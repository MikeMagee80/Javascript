...
//     for (let number = 2; number <= limit; number++) {  //starts from 2 because first prime number is 2 (divisible by 1 and 2)
//         // 2 - current (i)
//         let isPrime = true; 
//         for (let factor = 2; factor < number; factor++) { //inner loop, not including the number generated by the outer loop
//             if (number % factor === 0) { // if true, then not a Prime Number, so isPrime becomes false
//                 isPrime = false;
//                 break;
//             }
//         }
